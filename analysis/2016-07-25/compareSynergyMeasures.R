...
for(combo in c('6x6','10x10')){
  ...
  measure='CTG'
  for(cell in cells){
    ncats<-getNcatsSynergyScores(cell,measure,combo)
    recalc<-getreCalculateSynergyScores(cell,measure,combo)
    ...
    pheatmap(method.cors,cellwidth = 10,cellwidth=10,main=paste('Method correlations in',cell),filename=paste('MethodCorrelationsIn',gsub('\\)|\\(| ','',cell),'.png',sep=''))
